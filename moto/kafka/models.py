...
class FakeConfiguration(BaseModel):
    def __init__(self, name: str, account_id: str, region_name: str, description: str, kafka_versions: list[str], server_properties: str):
        self.name = name
        self.account_id = account_id
        self.region_name = region_name
...
class KafkaBackend(BaseBackend):
    ...
    def create_cluster_v2(self, cluster_name: str, tags: Optional[dict[str, str]], provisioned: Optional[dict[str, Any]], serverless: Optional[dict[str, Any]]) -> tuple[str, str, str, str]:
        if provisioned:
            ...
            raise BadRequestException("Either provisioned or serverless must be provided.")
        new_cluster = FakeKafkaCluster(cluster_name=cluster_name, account_id=self.account_id, region_name=self.region_name, cluster_type=cluster_type, broker_node_group_info=broker_node_group_info, kafka_version=kafka_version, number_of_broker_nodes=number_of_broker_nodes, serverless_config=serverless_config, tags=tags, state="CREATING", storage_mode=storage_mode if storage_mode else "LOCAL", current_version="1.0")
    ...
    def describe_cluster_v2(self, cluster_arn: str) -> dict[str, Any]:
        ...
        cluster_info: dict[str, Any] = {"activeOperationArn": "arn:aws:kafka:region:account-id:operation/active-operation", "clusterArn": cluster.arn, "clusterName": cluster.cluster_name, "clusterType": cluster.cluster_type, "creationTime": cluster.creation_time, "currentVersion": cluster.current_version, "state": cluster.state, "stateInfo": {"code": "string", "message": "Cluster state details."}, "tags": self.list_tags_for_resource(cluster.arn)}
        if cluster.cluster_type == "PROVISIONED":
            cluster_info["provisioned"] = {"brokerNodeGroupInfo": cluster.broker_node_group_info or {}, "clientAuthentication": cluster.client_authentication or {}, "currentBrokerSoftwareInfo": {"configurationArn": (cluster.configuration_info or {}).get("arn", "string"), "configurationRevision": (cluster.configuration_info or {}).get("revision", 1), "kafkaVersion": cluster.kafka_version}, "encryptionInfo": cluster.encryption_info or {}, "enhancedMonitoring": cluster.enhanced_monitoring, "openMonitoring": cluster.open_monitoring or {}, "loggingInfo": cluster.logging_info or {}, "numberOfBrokerNodes": cluster.number_of_broker_nodes or 0, "zookeeperConnectString": cluster.zookeeper_connect_string or "zookeeper.example.com:2181", "zookeeperConnectStringTls": cluster.zookeeper_connect_string_tls or "zookeeper.example.com:2181", "storageMode": cluster.storage_mode, "customerActionStatus": "NONE"}
        elif cluster.cluster_type == "SERVERLESS":
            cluster_info["serverless"] = {"vpcConfigs": cluster.serverless_config.get("vpcConfigs", []) if cluster.serverless_config else [], "clientAuthentication": cluster.serverless_config.get("clientAuthentication", {}) if cluster.serverless_config else {}}
        return cluster_info

    def list_clusters_v2(self, cluster_name_filter: Optional[str], cluster_type_filter: Optional[str], max_results: Optional[int], next_token: Optional[str]) -> tuple[list[dict[str, Any]], Optional[str]]:
        return [self.describe_cluster_v2(arn) for arn in self.clusters], None

    def create_cluster(self, broker_node_group_info: dict[str, Any], client_authentication: Optional[dict[str, Any]], cluster_name: str, configuration_info: Optional[dict[str, Any]] = None, encryption_info: Optional[dict[str, Any]] = None, enhanced_monitoring: str = "DEFAULT", open_monitoring: Optional[dict[str, Any]] = None, kafka_version: str = "2.8.1", logging_info: Optional[dict[str, Any]] = None, number_of_broker_nodes: int = 1, tags: Optional[dict[str, str]] = None, storage_mode: str = "LOCAL") -> tuple[str, str, str]:
        new_cluster = FakeKafkaCluster(cluster_name=cluster_name, account_id=self.account_id, region_name=self.region_name, cluster_type="PROVISIONED", broker_node_group_info=broker_node_group_info, client_authentication=client_authentication, kafka_version=kafka_version, number_of_broker_nodes=number_of_broker_nodes, configuration_info=configuration_info, encryption_info=encryption_info, enhanced_monitoring=enhanced_monitoring, open_monitoring=open_monitoring, logging_info=logging_info, storage_mode=storage_mode)
        self.clusters[new_cluster.arn] = new_cluster
        if tags:
            self.tag_resource(new_cluster.arn, tags)
    ...
    def get_cluster_policy(self, cluster_arn: str) -> dict[str, Any]:
        ...
        if cluster.cluster_policy is None:
            raise NotFoundException(f"No cluster policy found for cluster {cluster_arn}.")
        return {"currentVersion": cluster.cluster_policy_version, "policy": cluster.cluster_policy}

    def put_cluster_policy(self, cluster_arn: str, current_version: Optional[str], policy: str) -> dict[str, Any]:
        cluster = self._get_cluster(cluster_arn)
        cluster.cluster_policy = policy
        try:
            cluster.cluster_policy_version = str(int(cluster.cluster_policy_version) + 1)
        except ValueError:
    ...
    def delete_cluster_policy(self, cluster_arn: str) -> None:
        self._get_cluster(cluster_arn).cluster_policy = None
    ...
    def tag_resource(self, resource_arn: str, tags: dict[str, str]) -> None:
        self.tagger.tag_resource(resource_arn, [{"Key": k, "Value": v} for k, v in tags.items()])
