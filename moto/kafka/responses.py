...
class KafkaResponse(BaseResponse):
    ...
    def describe_cluster_v2(self) -> str:
        return json.dumps(
            {
                "clusterInfo": self.kafka_backend.describe_cluster_v2(
    ...
    def list_clusters_v2(self) -> str:
        cl, nt = self.kafka_backend.list_clusters_v2(
    ...
    def create_cluster(self) -> str:
        arn, name, state = self.kafka_backend.create_cluster(
    ...
    def describe_cluster(self) -> str:
        return json.dumps(
            {
                "clusterInfo": self.kafka_backend.describe_cluster(
                    cluster_arn=self._cluster_arn_from_path()
                )
            }
        )
    ...
    def delete_cluster(self) -> str:
        arn, state = self.kafka_backend.delete_cluster(
            cluster_arn=self._cluster_arn_from_path(),
    ...
    def list_clusters(self) -> str:
        cl = self.kafka_backend.list_clusters(
            cluster_name_filter=self._get_param("clusterNameFilter"),
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
        return json.dumps(
            {"clusterInfoList": cl, "nextToken": self._get_param("nextToken")}
        )

    def create_configuration(self) -> str:
        return json.dumps(
            self.kafka_backend.create_configuration(
    ...
    def delete_configuration(self) -> str:
        return json.dumps(
            self.kafka_backend.delete_configuration(
                arn=unquote(self.parsed_url.path).split("/v1/configurations/", 1)[1]
            )
        )

    def update_configuration(self) -> str:
        return json.dumps(
            self.kafka_backend.update_configuration(
                arn=unquote(self.parsed_url.path).split("/v1/configurations/", 1)[1],
                description=self._get_param("description") or "",
                server_properties=self._get_param("serverProperties") or "",
            )
    ...
    def list_configurations(self) -> str:
        configs, nt = self.kafka_backend.list_configurations(
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def list_configuration_revisions(self) -> str:
        ...
        revs, nt = self.kafka_backend.list_configuration_revisions(
            arn=arn,
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def describe_configuration_revision(self) -> str:
        ...
        return json.dumps(
            self.kafka_backend.describe_configuration_revision(
                arn=parts[0], revision=int(parts[1])
            )
        )

    def create_replicator(self) -> str:
        return json.dumps(
            self.kafka_backend.create_replicator(
                ...
                service_execution_role_arn=self._get_param("serviceExecutionRoleArn")
                or "",
                tags=self._get_param("tags"),
            )
        )
    ...
    def delete_replicator(self) -> str:
        return json.dumps(
            self.kafka_backend.delete_replicator(
                replicator_arn=unquote(self.parsed_url.path).split(
                    "/replication/v1/replicators/", 1
                )[1],
                current_version=self._get_param("currentVersion"),
            )
        )
    ...
    def list_replicators(self) -> str:
        reps, nt = self.kafka_backend.list_replicators(
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
    ...
    def update_replication_info(self) -> str:
        ...
        if "/replication-info" in ra:
            ra = ra.split("/replication-info")[0]
        return json.dumps(
            self.kafka_backend.update_replication_info(
                replicator_arn=ra,
                current_version=self._get_param("currentVersion"),
    ...
    def create_vpc_connection(self) -> str:
        return json.dumps(
            self.kafka_backend.create_vpc_connection(
    ...
    def describe_vpc_connection(self) -> str:
        return json.dumps(
            self.kafka_backend.describe_vpc_connection(
                arn=unquote(self.parsed_url.path).split("/v1/vpc-connection/", 1)[1]
            )
        )

    def delete_vpc_connection(self) -> str:
        return json.dumps(
            self.kafka_backend.delete_vpc_connection(
                arn=unquote(self.parsed_url.path).split("/v1/vpc-connection/", 1)[1]
            )
        )

    def list_vpc_connections(self) -> str:
        conns, nt = self.kafka_backend.list_vpc_connections(
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def list_client_vpc_connections(self) -> str:
        conns, nt = self.kafka_backend.list_client_vpc_connections(
            cluster_arn=self._cluster_arn_from_path(),
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def get_bootstrap_brokers(self) -> str:
        return json.dumps(
            self.kafka_backend.get_bootstrap_brokers(
                cluster_arn=self._cluster_arn_from_path()
            )
        )
    ...
    def delete_cluster_policy(self) -> str:
        self.kafka_backend.delete_cluster_policy(
            cluster_arn=self._cluster_arn_from_path()
        )
        return json.dumps({})
    ...
    def batch_associate_scram_secret(self) -> str:
        return json.dumps(
            self.kafka_backend.batch_associate_scram_secret(
                cluster_arn=self._cluster_arn_from_path(),
    ...
    def batch_disassociate_scram_secret(self) -> str:
        return json.dumps(
            self.kafka_backend.batch_disassociate_scram_secret(
                cluster_arn=self._cluster_arn_from_path(),
                secret_arn_list=self._get_param("secretArnList") or [],
            )
        )
    ...
    def list_scram_secrets(self) -> str:
        secrets, nt = self.kafka_backend.list_scram_secrets(
            cluster_arn=self._cluster_arn_from_path(),
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def list_nodes(self) -> str:
        nodes, nt = self.kafka_backend.list_nodes(
            cluster_arn=self._cluster_arn_from_path(),
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def list_kafka_versions(self) -> str:
        versions, nt = self.kafka_backend.list_kafka_versions(
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def get_compatible_kafka_versions(self) -> str:
        return json.dumps(
            {
                "compatibleKafkaVersions": self.kafka_backend.get_compatible_kafka_versions(
    ...
    def describe_cluster_operation(self) -> str:
        return json.dumps(
            {
                "clusterOperationInfo": self.kafka_backend.describe_cluster_operation(
                    cluster_operation_arn=unquote(self.parsed_url.path).split(
                        "/v1/operations/", 1
                    )[1]
                )
            }
    ...
    def describe_cluster_operation_v2(self) -> str:
        return json.dumps(
            {
                "clusterOperationInfo": self.kafka_backend.describe_cluster_operation_v2(
                    cluster_operation_arn=unquote(self.parsed_url.path).split(
                        "/api/v2/operations/", 1
                    )[1]
                )
    ...
    def list_cluster_operations(self) -> str:
        ops, nt = self.kafka_backend.list_cluster_operations(
            cluster_arn=self._cluster_arn_from_path(),
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def list_cluster_operations_v2(self) -> str:
        ops, nt = self.kafka_backend.list_cluster_operations_v2(
            cluster_arn=self._cluster_arn_from_path(),
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
        )
    ...
    def reboot_broker(self) -> str:
        return json.dumps(
            self.kafka_backend.reboot_broker(
                cluster_arn=self._cluster_arn_from_path(),
    ...
    def update_broker_count(self) -> str:
        return json.dumps(
            self.kafka_backend.update_broker_count(
                cluster_arn=self._cluster_arn_from_path(),
                current_version=self._get_param("currentVersion") or "",
                target_number_of_broker_nodes=int(
                    self._get_param("targetNumberOfBrokerNodes") or 3
                ),
            )
        )
    ...
    def update_broker_storage(self) -> str:
        return json.dumps(
            self.kafka_backend.update_broker_storage(
                cluster_arn=self._cluster_arn_from_path(),
                current_version=self._get_param("currentVersion") or "",
                target_broker_ebs_volume_info=self._get_param(
    ...
    def update_broker_type(self) -> str:
        return json.dumps(
            self.kafka_backend.update_broker_type(
                cluster_arn=self._cluster_arn_from_path(),
                current_version=self._get_param("currentVersion") or "",
    ...
    def update_cluster_configuration(self) -> str:
        return json.dumps(
            self.kafka_backend.update_cluster_configuration(
                cluster_arn=self._cluster_arn_from_path(),
    ...
    def update_cluster_kafka_version(self) -> str:
        return json.dumps(
            self.kafka_backend.update_cluster_kafka_version(
                cluster_arn=self._cluster_arn_from_path(),
                configuration_info=self._get_param("configurationInfo"),
                current_version=self._get_param("currentVersion") or "",
    ...
    def update_connectivity(self) -> str:
        return json.dumps(
            self.kafka_backend.update_connectivity(
                cluster_arn=self._cluster_arn_from_path(),
    ...
    def update_monitoring(self) -> str:
        return json.dumps(
            self.kafka_backend.update_monitoring(
                cluster_arn=self._cluster_arn_from_path(),
                current_version=self._get_param("currentVersion") or "",
                enhanced_monitoring=self._get_param("enhancedMonitoring"),
                open_monitoring=self._get_param("openMonitoring"),
    ...
    def update_security(self) -> str:
        return json.dumps(
            self.kafka_backend.update_security(
                cluster_arn=self._cluster_arn_from_path(),
                client_authentication=self._get_param("clientAuthentication"),
                current_version=self._get_param("currentVersion") or "",
                encryption_info=self._get_param("encryptionInfo"),
    ...
    def update_storage(self) -> str:
        return json.dumps(
            self.kafka_backend.update_storage(
                cluster_arn=self._cluster_arn_from_path(),
                current_version=self._get_param("currentVersion") or "",
    ...
    def update_rebalancing(self) -> str:
        return json.dumps(
            self.kafka_backend.update_rebalancing(
                cluster_arn=self._cluster_arn_from_path(),
                current_version=self._get_param("currentVersion") or "",
    ...
    def create_topic(self) -> str:
        return json.dumps(
            self.kafka_backend.create_topic(
                cluster_arn=self._cluster_arn_from_path(),
    ...
    def delete_topic(self) -> str:
        ...
        return json.dumps(
            self.kafka_backend.delete_topic(
                cluster_arn=self._cluster_arn_from_path(), topic_name=tn
            )
        )
    ...
    def describe_topic(self) -> str:
        ...
        tn = after_topics.split("/")[0] if "/" in after_topics else after_topics
        return json.dumps(
            self.kafka_backend.describe_topic(
                cluster_arn=self._cluster_arn_from_path(), topic_name=tn
            )
        )

    def list_topics(self) -> str:
        topics, nt = self.kafka_backend.list_topics(
            cluster_arn=self._cluster_arn_from_path(),
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
    ...
    def update_topic(self) -> str:
        ...
        tn = after_topics.split("/")[0] if "/" in after_topics else after_topics
        return json.dumps(
            self.kafka_backend.update_topic(
                cluster_arn=self._cluster_arn_from_path(),
                topic_name=tn,
                configs=self._get_param("configs"),
                partition_count=self._get_param("partitionCount"),
    ...
    def describe_topic_partitions(self) -> str:
        ...
        parts, nt = self.kafka_backend.describe_topic_partitions(
            cluster_arn=self._cluster_arn_from_path(),
            topic_name=tn,
            max_results=self._get_param("maxResults"),
            next_token=self._get_param("nextToken"),
