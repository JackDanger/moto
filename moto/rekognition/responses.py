...
class RekognitionResponse(BaseResponse):
    ...
    def create_collection(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.create_collection(body["CollectionId"])
        )

    def delete_collection(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.delete_collection(body["CollectionId"])
        )

    def describe_collection(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.describe_collection(body["CollectionId"])
        )

    def list_collections(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.list_collections(
    ...
    def index_faces(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.index_faces(
    ...
    def list_faces(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.list_faces(
                collection_id=body["CollectionId"],
                max_results=body.get("MaxResults", 4096),
                next_token=body.get("NextToken"),
            )
    ...
    def list_users(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.list_users(
                collection_id=body["CollectionId"],
    ...
    def delete_faces(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.delete_faces(
    ...
    def search_faces(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.search_faces(
                collection_id=body["CollectionId"],
                face_id=body["FaceId"],
    ...
    def search_faces_by_image(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.search_faces_by_image(
                collection_id=body["CollectionId"],
                image=body["Image"],
                max_faces=body.get("MaxFaces", 80),
                face_match_threshold=body.get("FaceMatchThreshold", 80.0),
    ...
    def describe_projects(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.describe_projects(
    ...
    def create_project_version(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.create_project_version(
    ...
    def describe_project_versions(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.describe_project_versions(
                project_arn=body["ProjectArn"],
    ...
    def delete_project_version(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.delete_project_version(body["ProjectVersionArn"])
        )

    def start_project_version(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.start_project_version(
    ...
    def stop_project_version(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.stop_project_version(body["ProjectVersionArn"])
        )
    ...
    def create_dataset(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.create_dataset(
                project_arn=body["ProjectArn"],
    ...
    def list_dataset_entries(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.list_dataset_entries(
    ...
    def list_dataset_labels(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.list_dataset_labels(
                dataset_arn=body["DatasetArn"],
                max_results=body.get("MaxResults", 100),
                next_token=body.get("NextToken"),
            )
    ...
    def create_stream_processor(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.create_stream_processor(
                name=body["Name"],
                input_config=body["Input"],
                output_config=body["Output"],
    ...
    def describe_stream_processor(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.describe_stream_processor(body["Name"])
        )

    def delete_stream_processor(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.delete_stream_processor(body["Name"])
        )

    def list_stream_processors(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.list_stream_processors(
                max_results=body.get("MaxResults", 100),
                next_token=body.get("NextToken"),
            )
        )
    ...
    def start_stream_processor(self) -> str:
        body = self._get_body()
        return json.dumps(
            self.rekognition_backend.start_stream_processor(
                name=body["Name"],
    ...
    def compare_faces(self) -> str:
        (face_matches, src_orient, tgt_orient, unmatched, src_face) = (
        ...
        return json.dumps(
            {
                ...
                "SourceImageFace": src_face,
            }
        )

    def detect_labels(self) -> str:
        (labels, image_properties, label_model_version) = (
    ...
    def detect_text(self) -> str:
        ...
        return json.dumps(
            {"TextDetections": text_detections, "TextModelVersion": text_model_version}
        )
