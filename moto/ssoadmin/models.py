...
class SSOAdminBackend(BaseBackend):
    ...
    def _assert_taggable_resource_exists(
        self, instance_arn: str, resource_arn: str
    ) -> None:
        """Raise ResourceNotFoundException unless `resource_arn` is a real permission
        set, application, or trusted token issuer — the taggable SSO Admin resource
        types. Real AWS validates the resource before tagging it (documented as a
        possible ResourceNotFoundException on TagResource/UntagResource/
        ListTagsForResource); an ARN-keyed dict alone can't tell "never tagged" from
        "does not exist"."""
        for finder, arg in (
            (self._find_permission_set, instance_arn),
            (self._find_application, None),
            (self._find_trusted_token_issuer, None),
        ):
            try:
                if arg is None:
                    finder(resource_arn)  # type: ignore[call-arg]
                else:
                    finder(arg, resource_arn)  # type: ignore[call-arg]
                return
    ...
    def tag_resource(
        ...
        self._assert_taggable_resource_exists(instance_arn, resource_arn)
