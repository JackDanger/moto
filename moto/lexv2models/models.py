...
class LexModelsV2Backend(BaseBackend, TaggableResourcesMixin):
    ...
    def create_bot_version(
        ...
        return {
            "botId": bot_id,
            "description": description,
            "botVersion": version.bot_version,
        ...
        return [
            {
                ...
                "botStatus": v.status,
                "creationDateTime": v.creation_date_time,
            }
    ...
    def delete_bot_version(
        self, bot_id: str, bot_version: str, skip_resource_in_use_check: bool
    ) -> tuple[str, str, str]:
        key = (bot_id, bot_version)
        if key not in self.bot_versions:
            raise ResourceNotFoundException(
    ...
    def describe_intent(
        ...
        return {
            ...
            "kendraConfiguration": intent.kendra_configuration,
            "botId": intent.bot_id,
            "botVersion": intent.bot_version,
            "localeId": intent.locale_id,
            "creationDateTime": intent.creation_date_time,
    ...
    def delete_intent(
        ...
        if key in self.bot_locales:
            self.bot_locales[key].intents_count = max(
                0, self.bot_locales[key].intents_count - 1
            )
    ...
    def list_intents(
        ...
        return [
            ...
            and intent.bot_version == bot_version
            and intent.locale_id == locale_id
        ]
    ...
    def list_slots(
        ...
        return [
            ...
            if slot.bot_id == bot_id
            and slot.bot_version == bot_version
            and slot.locale_id == locale_id
            and slot.intent_id == intent_id
        ]
    ...
    def create_slot_type(
        ...
        return {
            "slotTypeId": slot_type.slot_type_id,
            "slotTypeName": slot_type.slot_type_name,
            "description": slot_type.description,
            "slotTypeValues": slot_type.slot_type_values,
    ...
    def describe_slot_type(
        ...
        return {
            "slotTypeId": st.slot_type_id,
            "slotTypeName": st.slot_type_name,
            "description": st.description,
            "slotTypeValues": st.slot_type_values,
            "valueSelectionSetting": st.value_selection_setting,
            "parentSlotTypeSignature": st.parent_slot_type_signature,
            "botId": st.bot_id,
            "botVersion": st.bot_version,
            "localeId": st.locale_id,
    ...
    def delete_slot_type(
        ...
        if key in self.bot_locales:
            self.bot_locales[key].slot_types_count = max(
                0, self.bot_locales[key].slot_types_count - 1
            )
    ...
    def list_slot_types(
        ...
        return [
            ...
            if st.bot_id == bot_id
            and st.bot_version == bot_version
            and st.locale_id == locale_id
        ]
