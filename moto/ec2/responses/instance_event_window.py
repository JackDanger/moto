...
class InstanceEventWindowResponse(EC2BaseResponse):
    def create_instance_event_window(self) -> ActionResult:
        ...
        result = {"InstanceEventWindow": window}
        return ActionResult(result)

    def delete_instance_event_window(self) -> ActionResult:
        ...
        result = {
            "InstanceEventWindowId": window.id,
            "State": window.state,
        }
        return ActionResult(result)

    def describe_instance_event_windows(self) -> ActionResult:
        ...
        result = {"InstanceEventWindows": windows}
        return ActionResult(result)

    def modify_instance_event_window(self) -> ActionResult:
        window_id = self._get_param("InstanceEventWindowId")
        name = self._get_param("Name")
        time_ranges = self._get_param("TimeRange")
        cron_expression = self._get_param("CronExpression")
    ...
    def associate_instance_event_window(self) -> ActionResult:
        window_id = self._get_param("InstanceEventWindowId")
    ...
    def disassociate_instance_event_window(self) -> ActionResult:
        window_id = self._get_param("InstanceEventWindowId")
        assoc = self._get_param("AssociationTarget", {})
        instance_ids = assoc.get("InstanceId", [])
        dedicated_host_ids = assoc.get("DedicatedHostId", [])
