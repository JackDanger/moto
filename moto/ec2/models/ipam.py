...
class Ipam(TaggedEC2Resource):
    def __init__(
        ...
        self.id = ipam_id
        self.description = description
        self._operating_regions = operating_regions or [ec2_backend.region_name]
        self.tier = tier
        self.state = "create-complete"
    ...
    @property
    def operating_regions(self) -> list[dict[str, str]]:
        return [{"RegionName": region} for region in self._operating_regions]
    ...
    def modify_ipam(
        ...
        if add_operating_regions:
            for region in add_operating_regions:
                if region not in ipam._operating_regions:
